...
class DoubleBottomRSIStrategy(BaseStrategy):
    ...
    def find_local_minima(self, window: pd.DataFrame) -> List[Tuple[datetime, float]]:
        ...
        """
        _, lows = TrendAnalysis.detect_swing_points(
            window, lookback=3, price_col='low'  # Increased lookback for better bottom detection
        )
        
        minima = []
        for idx in window.index[lows]:
            minima.append((idx, window.loc[idx, 'low']))
    ...
    def detect_double_bottom_candidates(self) -> List[Dict]:
        ...
        # Detectar tendência (Series com -1, 0 ou 1) apenas para gerenciamento de risco
        trend = TrendAnalysis.detect_trend(self.df, strict=False)
